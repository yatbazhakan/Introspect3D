...
import pandas as pd
save_folder = "../custom_dataset/cnn_features_from_late/"
current_folder = "test"
...
def activation_hook(module, input, output):
    ...
    if isTrue:
        ...
        print(output.shape)
        isTrue=False
    torch.save(output,os.path.join(save_folder,current_folder,"features",current_file_name+".pt"))
...
if __name__ == "__main__":
    ...
    if(current_folder == "train"):
        ...
        print(f"Saving labels to {os.path.join(save_folder,current_folder,'labels.csv')} with shape {train_dataset_labels.shape}")
        ...
        with torch.no_grad():
            with tqdm(total=len(test_dataset)) as pbar:
                for i in range(len(test_dataset)):
                    ...
                    pbar.update(1)
        print(f"Saving labels to { os.path.join(save_folder,'labels.csv')} with shape {test_dataset_labels.shape}")
        ...
        with torch.no_grad():
            with tqdm(total=len(val_dataset)) as pbar:
                for i in range(len(val_dataset)):
                    ...
                    pbar.update(1)
        print(f"Saving labels to {os.path.join(save_folder,current_folder,'labels.csv')} with shape {val_dataset_labels.shape}")
