...
#TODO: Unit tests are needed for these classes with different data formats and datastes
class BoundingBox:
    def __init__(self,
    ...
    def remove_points_inside_box(self, point_cloud: np.ndarray) -> np.ndarray:
        ...
        - obbs        # Calculate yaw from the rotation matrix
        # Assuming the matrix is structured as follows:
        # self.rotation = [[R11, R12, R13],
        #                  [R21, R22, R23],
        #                  [R31, R32, R33]]: List of Open3D OrientedBoundingBox objects.
        ...
        if self.box_encoding == encoding:
            ...
            if self.box_encoding == "xywh":
                ...
                if self.img_width is None or self.img_height is None:
                    raise ValueError("Image dimensions are required for YOLO conversion")
                w = x2 - x1
                h = y2 -        # Calculate yaw from the rotation matrix
        # Assuming the matrix is structured as follows:
        # self.rotation = [[R11, R12, R13],
        #                  [R21, R22, R23],
        #                  [R31, R32, R33]] y1
