...
from cadcd_modules.config import cfg 
from cadcd_modules import cadc_calibration, dataset
from cadcd_modules.dataset import DatasetTemplate

import time

class cadcd(DatasetTemplate):
...
def create_cadc_infos(dataset_cfg, class_names, data_path, save_path, workers=8):
    dataset = cadcd(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
