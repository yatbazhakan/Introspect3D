...
def visualize_point_clouds(lidar=False, radar=False):
    idx = 0
    ...
    file_names = glob('/media/yatbaz_h/T7/HYY/*.npy')
    # Process Lidar data
    vis = o3d.visualization.Visualizer()
    vis.create_window()
    set_custom_view(vis)
    print(file_names)
    for lidar_filepath in file_names:
            
        pc_filter = EllipseFilter(a=15, b=35, offset=-5, axis=1)
        
        point_cloud = np.load(lidar_filepath)
        print(point_cloud.shape)
        point_cloud = point_cloud[point_cloud[:,0] != 1]
        point_cloud2 = np.concatenate((point_cloud, np.ones((len(point_cloud), 2))), axis=1)
        print(point_cloud.shape)
        inside_points = pc_filter.filter_pointcloud(point_cloud2, mode=FilteringArea.INSIDE)
        ...
        with open('errors_runrt.txt', 'a') as f:
            f.write(f"{lidar_filepath},{indice_max}\n")

        base_name = os.path.basename(lidar_filepath)
        # Run visualizer to ensure all data is loaded
        # input("Press Enter to continue...")
  
        vis.poll_events()
        vis.update_renderer()
        vis.capture_screen_image(f"outputs/demo/{base_name}_lidar_{idx}.png")
        idx += 1
