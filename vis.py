...
def set_custom_view(vis):
    ...
    # camera_params.extrinsic = np.array([[1, 0, 0, pcd_center[0]],
                                    # [0, 1, 0, pcd_center[1]],
                                    # [0, 0, 1, pcd_center[2] + 10],  # Adjust the '10' based on your point cloud scale
                                    # [0, 0, 0, 1]])
    ...
    extrinsic = np.eye(4)
    extrinsic[0:3, 3] = [0, 0, 70]  # Set camera position (x, y, z)
...
def compute_colors_from_distance(points,max_distance):
    ...
    colormap = plt.cm.get_cmap('viridis')  # Red to Yellow to Green
    # Map normalized distances to colors
    #set colors to cyan
    point_colors = np.zeros((points.shape[0], 4))
    point_colors[:, :3] = [0, 1, 1]
    # point_colors = colormap(normalized_distances)
    return point_colors[:,:3]
...
if __name__ == '__main__':
    ...
    with tqdm(total=7481) as pbar:
        for i in range(7481):
            ...
            print("Non Filtered points",points.shape)
            ...
            set_custom_view(vis)
            vis.run()
            output_filename = "angled_filtered.png"  # Define the filename and path as needed
